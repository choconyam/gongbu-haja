...
`scope`가 있으면 `../rules/incremental-notes.md`가 전체 검수 범위를 정한다. **이번 부분 전체**의 원자료 대응·논리·수식을 독립 검수하고, 이전 부분은 연결에 필요한 정의·기호·직전 절만 확인한다. 이전 승인 본문 전체를 재독·재검수하거나 현재 coverage에 다시 넣지 않는다. 실제 연결 검수 범위와 결과를 보고서에 남긴다.
