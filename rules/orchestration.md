...
두 모드의 새 기본 제작은 현재 세션의 모델·effort를 사용하는 직접 경로이며 이 문서의 역할 상태를 만들지 않는다. faithful은 `faithful-cost-path.md`, DEEP은 `deep-single-agent.md`를 따른다. 사용자 요청으로 새 관리형 실행을 시작하면 `init`은 `multi_agent`를 만든다. 아래 서브 에이전트·교육 보강·독립 검수·고비용 호출 의무는 `multi_agent` 및 해당 필드가 없는 기존 상태에 적용한다. 이미 기록된 DEEP `single_agent` 상태만 `main_agent`의 자체 점검 계약으로 이어간다. 관리형 `set-mode faithful`에서는 독립 검수로 전환하며 집필 이후 통과를 무효화한다. 기존 검수 이력은 삭제하지 않는다.

이 문서는 역할 프롬프트와 실제 에이전트 실행을 구분하고, 필요한 역할만 제한된 문맥으로 실행하는 방법을 정의한다.

관리자가 실행 계획·상태 전이·복구를 처리할 때 해당 절을 조회하는 운영 참고서다. 시작 시 전체 필독이 아니며 작성·내용 검수 담당에게 통째로 전달하지 않는다. 모델·effort는 `manage_run.py next --brief`로 확인하고, 이 문서의 다른 런타임 표를 반복 읽지 않는다. 아래 역할·호출·완료 게이트는 유지하며 참조 문서는 실제 작업과 연결된 절만 읽는다.
...
부분 제작·이어 쓰기는 `incremental-notes.md`를 먼저 따른다. `scope`가 있으면 이 문서의 전체 원자료·완성본은 이번 범위 전체와 필요한 앞부분 연결로 제한한다. 완료된 이전 실행에 새 진도를 `refresh-inputs`하지 않고 새 scoped 실행을 연결한다. 역할 모델·새 범위의 coverage·독립 검수 게이트는 그대로 유지한다.
