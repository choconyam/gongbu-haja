...
`deep` 작업에서는 `rules/deep-output-contract.md`를 반드시 읽고 집필·조판·검수에 적용한다. 최종 PDF, 쉬운 설명, 원본 교안 아래 해설, 교과서 수식, 최소 디자인이 기본이며 요청하지 않은 문제·정답·요약·제작 안내를 추가하지 않는다. 이 모드 계약은 아래 일반 디자인 권장보다 우선한다. `faithful`의 기본 Markdown 출력은 유지한다.
