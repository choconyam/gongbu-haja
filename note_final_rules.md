...
`deep` 작업에서는 `rules/deep-output-contract.md`를 반드시 읽고 집필·조판·검수에 적용한다. PDF 제작은 TeX 기준 원고 하나로 집필·수정·검수하며, 범위·전사 사전 확인과 재사용 가능한 대표 쪽 시험 조판으로 재작업을 줄인다. 최종 PDF, 쉬운 설명, 원본 교안 아래 해설, 교과서 수식, 최소 디자인이 기본이며 요청하지 않은 문제·정답·요약·제작 안내를 추가하지 않는다. 이 모드 계약은 아래 일반 디자인 권장보다 우선한다. `faithful`의 기본 Markdown 출력은 유지한다.

사용자가 새 학습노트를 요청하면서 모드를 명시하지 않았다면 관리자는 다른 작업을 시작하기 전에 두 선택지를 짧게 설명하고 반드시 선택을 받는다. 추천은 가능하지만 과목 계열로 대신 결정하지 않는다. 명령행 초기화의 `--note-mode`는 필수 인자라, 선택을 받기 전에는 실행 상태를 만들 수 없다.
...
비용과 품질은 제작 모드별로 분리한다. `faithful` 집필도 `quality_high`(상위 모델)이고, 최종 source-unit 누락·왜곡·약화 대조는 독립 `review_high`(상위 모델 `high`)가 담당한다. `deep` 집필·의미 보강은 `quality_high`, 완성본 전체의 독립 논리·유도 검수 1회는 `quality_xhigh`가 담당한다. 전사 후보 판정·자료 대응·조판 표본과 Python으로 확정할 수 있는 단계에 `quality_*` 프로필을 사용하지 않으며, 런타임 모델표에 없는 상위 모델은 실행 프로필에 두지 않는다.

최종 `review_high` 또는 `quality_xhigh` 완성본 검수는 상태 파일의 현재 `review_cycle`마다 한 번만 시작할 수 있다. 검수에서 발견한 국소 문제는 같은 호출 안에서 패치하고 바뀐 위치만 다시 확인한다. 입력·제작 모드·사용자가 명시한 편집 계약이 실제로 바뀐 경우에만 새 cycle을 연다.
...
교안을 여러 수업에 나누어 진행할 때는 `rules/incremental-notes.md`를 우선 적용한다. 이번 진도만 별도 실행으로 제작하고 이전 승인 원고·검수를 재사용한다. 아래의 모든 자료·완성본 검수는 이번 범위와 앞부분과의 연결을 뜻하며, 누적 출력의 전체 시각 확인은 유지한다.
...
10. 조판은 `scripts/build_study_note_pdf.py`가 초안을 내용 그대로 요청 형식으로 만들고 `scripts/validate_note_output.py`를 통과시킨다. 관리자가 렌더 표본을 확인한다. 독립 최종 검수는 이 단계와 병렬로 집필 초안을 검수한다.
...
- 별도 형식이 없으면 `deep`은 `rules/deep-output-contract.md`의 최소 조판을, 그 외에는 `rules/output-and-layout.md`의 adaptive 프로필을 사용한다.
