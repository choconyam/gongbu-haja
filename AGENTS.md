...
- 배포 기본은 Git clone/pull이고 전역 CLI는 선택 기능이다. CLI·패키징을 수정하지 않은 업데이트에서 패키지 빌드·재설치를 반복하지 않는다.
- 문서·역할 지시 수정은 `validate_agent_setup.py --strict`와 diff 확인, 모델표 수정은 선언 동기화와 `test_execution_profiles`·`test_sync_runtime_agents`·`test_manage_run`으로 검증한다.
- 실행 코드 수정은 해당 모듈과 직접 연결된 테스트만 로컬에서 실행한다. PDF 빌더·템플릿 수정 때만 관련 빌드·렌더 검사를, CLI·경로·패키징 수정 때만 설치 검사를 추가한다. 여러 범주를 수정했다면 필요한 검사를 합친다.
- 전체 호환성 검증은 `.github/workflows/validate.yml`이 담당한다. 실패·공통 계약 변경 등 구체적 근거가 있을 때만 로컬 검사 범위를 넓힌다. 성공 로그는 요약하고 실패 부분만 상세히 읽는다. 로컬 통과와 CI 통과를 혼동하지 않는다.
- 배포 확인을 위해 실제 강의를 비교 집필하거나 의미 검수를 추가 호출하지 않는다. 아래 학습노트 제작의 내용 검수·완료 게이트는 그대로 적용한다.

## 실행 원칙

- 현재 진도까지만 제작하거나 뒤를 이어 만드는 요청은 `rules/incremental-notes.md`를 먼저 적용한다. 첫 부분부터 `--scope`, 다음 부분은 `--continue-from`으로 분리하고 승인된 앞부분을 재집필·전체 재검수하지 않는다. 이 경우 아래의 전체 자료·완성본 검수·입력 추가 규칙은 해당 문서의 범위 계약으로 제한한다.

- 새 `faithful`은 `preprocessing=deterministic`으로 초기화한다. `rules/faithful-cost-path.md`가 아래의 일반 전사 검수·자료 매핑 hybrid 경로보다 우선한다. 전사와 원문 분할·후보 표시는 로컬, 기본 의미 담당은 작성 1개와 독립 최종 검수 1개다. 원문의 모든 예시·조건을 두 담당이 대조하며 의미 충돌이 남을 때만 국소 검수를 추가한다. 기존 실행과 신규 `deep`의 정책은 유지한다.
...
```powershell
python scripts/manage_run.py complete workspace/<강의ID>/run_state.json --role final_reviewer --artifact <검수_결과> --source-map <source_map_JSON> --coverage-report <coverage_report_JSON> --patched <기준_원고.md_또는.tex>
