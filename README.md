...
새 **자료 충실형**은 로컬 Whisper 전사 → Python 원문 분할·오류 후보 표시 → **작성 담당 1개 + 독립 검수 담당 1개**가 기본이다. `gongbu prepare-sources`는 모든 원문과 근거 위치를 보존하고, 두 담당이 정의·예시·조건의 누락을 확인한다. 자동 검사는 음성·의미 검증으로 표시하지 않는다. 불명확한 핵심 구간만 추가 검수한다. 아래 일반 흐름의 전사 검수·자료 매핑 담당은 이 경로에서 Python으로 실행되며, 기존 실행과 신규 `deep`의 의미 대응 경로는 유지한다. [자료 충실형 최소 모델 경로](rules/faithful-cost-path.md)에 실행 예시와 품질 기준이 있다.
...
- 자료 충실형은 중간 의미 요약 담당을 기본 생략하고 원문을 작성·독립 검수에 전달한다. 관리자는 전체 전사·초안을 재독하지 않고 경로·개수·해시·오류 요약만 읽는다.
- 상태 확인은 `run next <상태_JSON> --brief`로 필요한 역할 정보만 받는다. 전체 대화 상속과 noop 에이전트를 금지하고 독립 읽기·검증은 묶어서 실행한다. 담당 2개가 모델 응답 2회를 뜻하지는 않으므로 실제 호출·토큰은 다음 작업 로그로 비교한다.
