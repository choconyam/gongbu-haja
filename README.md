...
> [!TIP]
> **최신 업데이트 · v1.7.3 — 필요한 구간만 다시 듣는 전사 검수**
>
> 전사는 강의를 두 번 듣게 하려고 만드는 게 아니니까요. 교안과 문맥으로 먼저 바로잡고, 학습 내용에 영향을 주는 구간만 원음에서 확인합니다. 원본과 재청취 정보는 그대로 보존합니다. [변경 전후 자세히 보기](CHANGELOG.md)
...
**녹음이 인터넷에 올라가나요?** 전사는 로컬 Whisper로 돌아가서 원본 녹음을 외부 전사 서비스에 올리지 않습니다. 노트에는 타임스탬프와 반복 화자 표지를 뺀 간결형 전사를 사용하고, 교안으로도 풀리지 않는 핵심 표현만 원음을 다시 확인합니다. 다만 노트를 만드는 동안 AI 도구가 읽은 텍스트와 이미지의 처리 방식은 Codex, Claude Code, Cursor의 계정 설정과 서비스 정책을 따릅니다. ([더 보기](docs/install.md#자주-묻는-것))
...
| [패치노트](CHANGELOG.md) | 최신 `v1.7.3`과 버전별 변경 사항, Before/After |
